...
function addBlackBackround() {
    sayHelloButton.classList.add('blackBackround');
}
...
function removeBlackBackround() {
    sayHelloButton.classList.remove('blackBackround');
}

sayHelloButton.addEventListener('mouseout', function(event) {
    removeBlackBackround();
})
...
// CODE HERE
sayHelloButton.addEventListener('click', function(eve) {
    sayHello();
})
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/"helloguys"')
        .then(function(res) {
            console.log(res);
...
function queryRequest() {
    axios.get('http://localhost:3000/query-test?fruit=banana')
        .then(function(res) {
...
/*
    ...
    1: Send no queries on the URL -- what happened? nothing
...
document.querySelector('#input-Button').addEventListener('click', function(eve) {
    createFood();
})